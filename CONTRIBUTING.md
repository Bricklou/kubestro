...
Before opening a feature request, please ensure that the feature is not already in the roadmap. You can check the [project board](https://github.com/users/Bricklou/projects/9) to see the current status of the project.
...
The project is divided into 4 sub-projects: [api](./projects/api/), [operator](./projects/operator/), [download](./projects/download/) and [webui](./projects/webui/).
...
For all the sub-projects, you need to have [Task](https://taskfile.dev/#/) installed on your machine. You can install it by following the instructions on the website.

---

For the web UI, you need to enable `corepack` to install dependencies. You can do this by running the following command:
...
For the other sub-projects, you can install the dependencies by running the following command:

```bash
task install
```

5. Start the project on your local machine.

```bash
task dev
```

This will start the project in development mode. You can now start coding and see the changes in real-time.
